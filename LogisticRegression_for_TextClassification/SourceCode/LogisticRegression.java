...
class Instance {
    ...
    /** String表示文本中的一个单词， Double表示该单词对应的TF-IDF值*/
...
public class LogisticRegression {
    ...
    private double regularParam = 0.00;
    */
    ...
    public LogisticRegression(String isRemoveStopWords, double LearningRate, int Iterations, double RegularParam) {
	...
	public void testLogisticRegression() {	
        ...
        for (int i=0; i<Instances.size(); i++) {
            ...
            int label = Instances.get(i).label;
            if(label == readFiles.PositiveLable && predicted < 0.5)
            	ErrorCount++;
            else if(label == readFiles.PositiveLable && predicted >= 0.5)
            	CorrectCount++;
            else if(label == readFiles.NegativeLable && predicted >= 0.5)
            	ErrorCount++;
            else if(label == readFiles.NegativeLable && predicted < 0.5)
            	CorrectCount++;
            else
	 ...
	 * @return 并返回Instances列表，每一列代表一个TF*IDF向量，即一个样本
	 **/
